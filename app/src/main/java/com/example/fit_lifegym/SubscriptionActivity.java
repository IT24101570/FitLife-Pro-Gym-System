...
public class SubscriptionActivity extends AppCompatActivity {
    ...
    private void setupListeners() {
        ...
        rgBillingCycle.setOnCheckedChangeListener((group, checkedId) -> {
            if (checkedId == R.id.rbMonthly) {
                selectedCycle = getString(R.string.label_monthly);
                rbMonthly.setTextColor(getResources().getColor(R.color.gold_primary));
                rbYearly.setTextColor(getResources().getColor(R.color.text_secondary));
            } else {
                selectedCycle = getString(R.string.label_yearly);
                rbYearly.setTextColor(getResources().getColor(R.color.gold_primary));
    ...
    private void updatePrices() {
        if (selectedCycle.equals(getString(R.string.label_monthly))) {
            tvBasicPrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getMonthlyPrice(Subscription.Plan.BASIC))));
            tvPremiumPrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getMonthlyPrice(Subscription.Plan.PREMIUM))));
            tvElitePrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getMonthlyPrice(Subscription.Plan.ELITE))));
        } else {
            tvBasicPrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getYearlyPrice(Subscription.Plan.BASIC))));
            tvPremiumPrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getYearlyPrice(Subscription.Plan.PREMIUM))));
            tvElitePrice.setText(getString(R.string.price_format, String.valueOf(Subscription.Plan.getYearlyPrice(Subscription.Plan.ELITE))));
        }
    }
                ...
                if (snapshot.exists()) {
                    for (DataSnapshot data : snapshot.getChildren()) {
                        ...
                        if (sub != null && sub.isActive()) {
                            btnSubscribe.setText(getString(R.string.label_current_plan, sub.getPlanType()));
                            break;
                        }
                    }
                }
    ...
    private void proceedToPayment() {
        double price = selectedCycle.equals(getString(R.string.label_monthly))
